...
export function SignInForm({}: Props) {
  ...
  const showAccountCreatedSuccessMsg = router.query["signup-succeeded"] === "1";
  ...
  return (
    <form className={styles.root} onSubmit={f.handleSubmit}>
      ...
      <Button mt="4" mb="4" type="submit" disabled={f.isSubmitting} loading={f.isSubmitting}>
        Sign in
      </Button>

      <Box mb="4">
        <Text size="2">
          {"Don't"} have an account? <Link href={paths.signup}>Set up one.</Link>
        </Text>
      </Box>
