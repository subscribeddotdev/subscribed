...
type organizationR struct {
	OrgAPIKeys    APIKeySlice      `boil:"OrgAPIKeys" json:"OrgAPIKeys" toml:"OrgAPIKeys" yaml:"OrgAPIKeys"`
	Environments  EnvironmentSlice `boil:"Environments" json:"Environments" toml:"Environments" yaml:"Environments"`
	OrgEventTypes EventTypeSlice   `boil:"OrgEventTypes" json:"OrgEventTypes" toml:"OrgEventTypes" yaml:"OrgEventTypes"`
	Members       MemberSlice      `boil:"Members" json:"Members" toml:"Members" yaml:"Members"`
	OrgMessages   MessageSlice     `boil:"OrgMessages" json:"OrgMessages" toml:"OrgMessages" yaml:"OrgMessages"`
}
...
func (r *organizationR) GetOrgAPIKeys() APIKeySlice {
	if r == nil {
		return nil
	}
	return r.OrgAPIKeys
...
func (r *organizationR) GetOrgEventTypes() EventTypeSlice {
	if r == nil {
		return nil
	}
	return r.OrgEventTypes
}
...
func (r *organizationR) GetOrgMessages() MessageSlice {
	if r == nil {
		return nil
	}
	return r.OrgMessages
}
...
func (o *Organization) OrgEventTypes(mods ...qm.QueryMod) eventTypeQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"event_types\".\"org_id\"=?", o.ID),
	)
...
func (o *Organization) OrgMessages(mods ...qm.QueryMod) messageQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"messages\".\"org_id\"=?", o.ID),
	)

	return Messages(queryMods...)
}

// LoadOrgAPIKeys allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (organizationL) LoadOrgAPIKeys(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`api_keys`),
		qm.WhereIn(`api_keys.org_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.OrgAPIKeys = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.OrgID {
				local.R.OrgAPIKeys = append(local.R.OrgAPIKeys, foreign)
				if foreign.R == nil {
					foreign.R = &apiKeyR{}
				}
				foreign.R.Org = local
...
func (organizationL) LoadEnvironments(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`environments.organization_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadOrgEventTypes allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (organizationL) LoadOrgEventTypes(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	var slice []*Organization
	var object *Organization
	...
	query := NewQuery(
		qm.From(`event_types`),
		qm.WhereIn(`event_types.org_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice event_types")
	}
	...
	if singular {
		object.R.OrgEventTypes = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.OrgID {
				local.R.OrgEventTypes = append(local.R.OrgEventTypes, foreign)
				if foreign.R == nil {
					foreign.R = &eventTypeR{}
				}
				foreign.R.Org = local
...
func (organizationL) LoadMembers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`members`),
		qm.WhereIn(`members.organization_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadOrgMessages allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (organizationL) LoadOrgMessages(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrganization interface{}, mods queries.Applicator) error {
	var slice []*Organization
	var object *Organization
	...
	query := NewQuery(
		qm.From(`messages`),
		qm.WhereIn(`messages.org_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.OrgMessages = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &messageR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.OrgID {
				local.R.OrgMessages = append(local.R.OrgMessages, foreign)
				if foreign.R == nil {
					foreign.R = &messageR{}
				}
				foreign.R.Org = local
...
// AddOrgAPIKeys adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.OrgAPIKeys.
...
func (o *Organization) AddOrgAPIKeys(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*APIKey) error {
	...
	if o.R == nil {
		...
		o.R.OrgAPIKeys = append(o.R.OrgAPIKeys, related...)
	}
...
// AddOrgEventTypes adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.OrgEventTypes.
// Sets related.R.Org appropriately.
func (o *Organization) AddOrgEventTypes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*EventType) error {
	...
	if o.R == nil {
		o.R = &organizationR{
			OrgEventTypes: related,
		}
	} else {
		o.R.OrgEventTypes = append(o.R.OrgEventTypes, related...)
	}
...
// AddOrgMessages adds the given related objects to the existing relationships
// of the organization, optionally inserting them as new records.
// Appends related to o.R.OrgMessages.
// Sets related.R.Org appropriately.
func (o *Organization) AddOrgMessages(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Message) error {
	...
	if o.R == nil {
		...
		o.R.OrgMessages = append(o.R.OrgMessages, related...)
	}
