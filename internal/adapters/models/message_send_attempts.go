...
func (messageSendAttemptL) LoadEndpoint(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMessageSendAttempt interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`endpoints`),
		qm.WhereIn(`endpoints.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *MessageSendAttempt) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(messageSendAttemptAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(messageSendAttemptPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert message_send_attempts, could not build conflict column list")
			}

			conflict = make([]string, len(messageSendAttemptPrimaryKeyColumns))
			copy(conflict, messageSendAttemptPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"message_send_attempts\"", updateOnConflict, ret, update, conflict, insert, opts...)
