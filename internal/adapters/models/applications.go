...
func (applicationL) LoadEnvironment(ctx context.Context, e boil.ContextExecutor, singular bool, maybeApplication interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		args[object.EnvironmentID] = struct{}{}
	...
	query := NewQuery(
		...
		qm.WhereIn(`environments.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (applicationL) LoadEndpoints(ctx context.Context, e boil.ContextExecutor, singular bool, maybeApplication interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`endpoints`),
		qm.WhereIn(`endpoints.application_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (applicationL) LoadMessages(ctx context.Context, e boil.ContextExecutor, singular bool, maybeApplication interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`messages`),
		qm.WhereIn(`messages.application_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
